<#
=============================================================================================
Name:           Audit File Access in SharePoint Online using PowerShell
Version:        1.0
...
3. The script retrieves file access audit log for 180 days, by default.
4. Allows you generate file access audit report for a custom period. 
5. Helps to find recently accessed files. e.g., Files accessed in the last 30 days. 
6. Exports report results to CSV file.     
7. Identifies file accessed by external/guest users. 
8. Helps to monitor all files accessed by a specific user.
9. Helps to track SharePoint Online file accesses alone
10.Tracks OneDrive file accesses alone.
11.Finds files accessed by a specific person in the specific period   
...
For detailed script execution: https://o365reports.com/2024/01/30/audit-file-access-in-sharepoint-online-using-powershell 
============================================================================================
#>
...
if($RecentlyAccessedFiles_In_Days -ne "")
{
...
$OutputCSV=".\Audit_File_Access_Report$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" 
...
{ 
 ...
 foreach($Result in $Results)
 {
  ...
  {
   $OutputEvents++
  $ExportResult=@{'Accessed Time'=$ActivityTime;'Accessed By'=$UserID;'Workload'=$Workload;'More Info'=$MoreInfo;'Accessed File'=$AccessedFile;'Site URL'=$SiteURL;'File Extension'=$FileExtension}
  $ExportResults= New-Object PSObject -Property $ExportResult  
  $ExportResults | Select-Object 'Accessed Time','Accessed By','Accessed File','Site URL','File Extension','Workload','More Info' | Export-Csv -Path $OutputCSV -Notype -Append 
 }
 }
 Write-Progress -Activity "`n     Retrieving file access audit data from $StartDate to $EndDate.."`n" Processed audit record count: $i"
